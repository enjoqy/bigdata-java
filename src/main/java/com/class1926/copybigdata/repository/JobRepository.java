...
public interface JobRepository extends JpaRepository<Job, Long> {
     ...
     * @return
     */
    @Query(value = "select address as city,AVG(salary) as avgByCity,SUM(hiring) as countByCity,AVG(experience) as experience,SUM(salary) as budget FROM job_java where if(?1 != '',type =?,1=1) group by address", nativeQuery = true)
     ...
     * @return
     */
    @Query(value = "select province,AVG(salary) as avgByProvince,SUM(hiring) as countByProvince,AVG(experience) as experience,SUM(salary) as budget FROM job_java where if(?1 != '',type =?,1=1)  group by province", nativeQuery = true)
     ...
     * @return
     */
    @Query(value = "SELECT education,count(hiring) FROM job_java where if(?1 != '',type =?,1=1) GROUP BY education",nativeQuery = true)
